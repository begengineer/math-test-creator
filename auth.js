...
class AuthManager {
    constructor() {
        // 環境変数から設定を読み込み
        this.jwtSecret = process.env.JWT_SECRET || 'math-test-creator-secret-key-2025';
        ...
        // 本番環境での警告
        if (process.env.NODE_ENV === 'production' && this.jwtSecret === 'math-test-creator-secret-key-2025') {
            console.warn('⚠️  WARNING: Using default JWT secret in production!');
        }
    }

    // デフォルトパスワード "mathtest2025" のハッシュを生成
    generateDefaultHash() {
        const defaultPassword = 'mathtest2025';
